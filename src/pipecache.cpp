...
{
    return do_read(ksceKernelMemcpyToUser, udata, max_size, timeout);
}

int PipeCache::copyin(std::uintptr_t udata, size_t size)
{
    return do_write(ksceKernelMemcpyFromUser, udata, size);
}
...
void PipeCache::read_cache(F f, Ptr data, std::size_t size)
{
    ...
    {
        f(reinterpret_cast<void *>(data), &m_base[m_start_ptr], end_copy_size);
        f(reinterpret_cast<void *>(data + end_copy_size), m_base, size - end_copy_size);
    }

    // no overlap with end of buffer, just copy straight
    else
    {
        f(reinterpret_cast<void *>(data), &m_base[m_start_ptr], size);
    }
...
void PipeCache::write_cache(F f, Ptr data, std::size_t size)
{
    ...
    if (size > end_copy_size) 
    {
        f(&m_base[m_end_ptr], reinterpret_cast<const void *>(data), end_copy_size);
        f(m_base, reinterpret_cast<const void *>(data + end_copy_size), size - end_copy_size);
    }

    // no overlap with end of buffer, just copy straight
    else
    {
        f(&m_base[m_end_ptr], reinterpret_cast<const void *>(data), size);
    }
